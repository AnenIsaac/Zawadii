...
import { View, Text, ScrollView, TouchableOpacity, StyleSheet, ActivityIndicator, Image, Alert, Modal, Pressable } from 'react-native';
...
export default function RewardsScreen() {
  ...
  const [modalPayload, setModalPayload] = useState({ image: '', title: '', code: '' });
  ...
  if (!authUser) {
    return <Text style={styles.emptyText}>Please sign in to view your rewards.</Text>;
  }
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {customerRewards.map(cr => (
        <View key={cr.id} style={styles.card}>
          {/* 1) IMAGE */}
          <Image source={{ uri: cr.reward.image_url }} style={styles.cardImage} />
            ...
            <View style={styles.cardMeta}>
              ...
              {cr.status === 'bought' && (
                <Text style={styles.cardSubtitle}>
                  Bought at {new Date(cr.claimed_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                </Text>
              )}
              {cr.status === 'redeemed' && (
                <Text style={styles.cardSubtitle}>
                  Redeemed at {new Date(cr.redeemed_at).toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}
                </Text>
              )}
            </View>
          </View>
      ...
      {!customerRewards.length && <Text style={styles.emptyText}>No rewards yet.</Text>}
